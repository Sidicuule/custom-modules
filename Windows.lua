...
local autoac = COB("Blatant", {
	["Name"] = "AutoAnticheatDisabler",
	["Function"] = function(callback)
		if callback then
			pcall(function()
				ScriptSettings.autoac = true
				while task.wait() do
					if not ScriptSettings.autoac == true then return end
					game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
        queue_on_teleport("loadstring(game:HttpGet('https://windowscustom.000webhostapp.com/acdisabler.lua'))()")
    end
end)
				end
			end)
			...
			pcall(function()
				ScriptSettings.autoac = false
				game:GetService("Workspace").Gravity = 196.2
			end)
		end
	end,
	["Default"] = false,
	["HoverText"] = "runs anticheat disabler the next time you tp"
})
